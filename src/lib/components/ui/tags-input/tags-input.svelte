<script lang="ts">
  ...
  let inputValue = $state('');
	let asyncSuggestions = $state<string[]>([]);
...
const filteredSuggestions = $derived.by(() => {
		if (fetchSuggestions) {
			return asyncSuggestions.filter((s) => !value.includes(s));
		}
...
>
	...
	{#if showSuggestions}
		...
		>
      {#if isLoadingSuggestions && filteredSuggestions.length === 0}
        <div class="text-muted-foreground px-2 py-1.5 text-sm">Searching...</div>
      {/if}
